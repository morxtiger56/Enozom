...
export class BoardDB {
    ...
    async getBoardById(id: number) {
        try {
            ...
            const board = await connection.manager.findOne(Board, options);
            return board;
        } catch (error) {
    ...
    public static async getBoardElementByBoardIdAndStart(
        ...
        try {
            ...
            const ele = await connection.manager.findOne(
                Board_Elements,
                options
            );
    ...
    public static async getRandomBoard(): Promise<number> {
        try {
            const connection = await ConnectionManager.getConnection();
            const count = await connection.manager.count(Board);
            const board = await connection.manager.find(Board, {
                where: { id: Not(IsNull()) },
