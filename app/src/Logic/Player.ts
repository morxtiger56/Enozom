...
export default class Player {
  ...
  public static async changesPerMove(
    ...
    let comingUser;
    comingUser = await GameUserDB.getGameUserByGameIdAndTurnOrder(
      gameId,
      nextTurnOrder
    );
  ...
  public static async moveMyPlayer(userId: number, game: Game) {
    ...
    if (game.state != 'end') {
      try {
        ...
        try {
          currentElement = await BoardDB.getBoardElementByBoardIdAndStart(
            id_board,
            newPosition
          );
          ...
          let nexTID = await this.changesPerMove(
            game.id,
            userId,
            newPosition,
            nextTurnOrder,
